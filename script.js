...
if(song && playMusic){
  ...
  stopMusic.addEventListener('click', () => {
    ...
    playMusic.textContent='▶'; playMusic.classList.remove('is-playing'); vinyl.classList.remove('is-playing');
  });
  song.addEventListener('timeupdate', () => { if(song.duration) musicProgress.style.width=(song.currentTime/song.duration*100)+'%'; });
  song.addEventListener('ended', () => { playMusic.textContent='▶'; playMusic.classList.remove('is-playing'); vinyl.classList.remove('is-playing'); musicProgress.style.width='0%'; });
}
...
// Subtle pointer parallax for the ambient background (visual only; safe if elements are missing).
(() => {
  let raf = 0;
  document.addEventListener('pointermove', (e) => {
    ...
    raf = requestAnimationFrame(() => {
      document.body.style.setProperty('--pointer-x', x.toFixed(2) + 'px');
      document.body.style.setProperty('--pointer-y', y.toFixed(2) + 'px');
    });
  }, {passive:true});
})();
