...
const styles=[
 {id:'bold',name:'Жирный',cat:'simple',fn:t=>t,css:'font-weight:700;',rich:true,supports:{ru:true,digits:true}},
 {id:'italic',name:'Курсивный',cat:'simple',fn:t=>applyMap(t,maps.italic),supports:{ru:false,digits:false}},
 {id:'boldItalic',name:'Жирный курсивный',cat:'simple',fn:t=>t,css:'font-weight:700;font-style:italic;',rich:true,supports:{ru:true,digits:true}},
 {id:'mono',name:'Моноширинный',cat:'simple',fn:t=>applyMap(t,maps.mono),supports:{ru:false,digits:true}},
 {id:'double',name:'Двойной контур',cat:'decorative',fn:t=>applyMap(t,maps.double),supports:{ru:false,digits:true}},
 {id:'gothic',name:'Готический',cat:'decorative',fn:t=>applyMap(t,maps.gothic),supports:{ru:false,digits:false}},
 {id:'gothicBold',name:'Жирный готический',cat:'decorative',fn:t=>applyMap(t,maps.gothicBold),supports:{ru:false,digits:false}},
 {id:'underline',name:'Подчёркнутый',cat:'decorative',fn:t=>decorate(t,'underline'),supports:{ru:true,digits:true}},
 {id:'doubleline',name:'Двойное подчёркивание',cat:'decorative',fn:t=>decorate(t,'doubleline'),supports:{ru:true,digits:true}},
 {id:'strike',name:'Перечёркнутый',cat:'decorative',fn:t=>decorate(t,'strike'),supports:{ru:true,digits:true}},
 {id:'overline',name:'Надчёркнутый',cat:'decorative',fn:t=>decorate(t,'overline'),supports:{ru:true,digits:true}},
 {id:'dot',name:'Точечный',cat:'symbols',fn:t=>decorate(t,'dot'),supports:{ru:true,digits:true}},
 {id:'dia',name:'Диакритический',cat:'symbols',fn:t=>decorate(t,'dia'),supports:{ru:true,digits:true}},
 {id:'spaced',name:'Просторный',cat:'symbols',fn:spaced,supports:{ru:true,digits:true}},
 {id:'circled',name:'Кружочки',cat:'symbols',fn:circled,supports:{ru:false,digits:false}},
 {id:'square',name:'Квадратные скобки',cat:'symbols',fn:square,supports:{ru:true,digits:true}},
 {id:'fullwidth',name:'Широкий',cat:'symbols',fn:fullwidth,supports:{ru:true,digits:true}}
];
...
function renderFonts(){
  ...
  list.forEach(style=>{
    ...
    const badges=document.createElement('div'); badges.className='font-supports';
    const ru=document.createElement('span'); ru.className='font-support '+(style.supports.ru?'yes':'no'); ru.title=style.supports.ru?'Русские буквы поддерживаются':'Русские буквы не изменяются'; ru.textContent=(style.supports.ru?'✓':'✕')+' RU';
    const dg=document.createElement('span'); dg.className='font-support '+(style.supports.digits?'yes':'no'); dg.title=style.supports.digits?'Цифры поддерживаются':'Цифры не изменяются'; dg.textContent=(style.supports.digits?'✓':'✕')+' 123';
    badges.append(ru,dg); top.append(name,badges); left.appendChild(top);
    const preview=document.createElement('div'); preview.className='font-preview'; preview.textContent=style.fn(text); if(style.css) preview.style.cssText += style.css; left.appendChild(preview);
    ...
    btn.addEventListener('click',async()=>{
      ...
      try {
        if(style.rich && navigator.clipboard && window.ClipboardItem){
          const html=`<span style=\"${style.css}font-family:inherit;\">${escapeHtml(value).replace(/\n/g,'<br>')}</span>`;
          const item=new ClipboardItem({'text/plain':new Blob([value],{type:'text/plain'}),'text/html':new Blob([html],{type:'text/html'})});
          await navigator.clipboard.write([item]);
        } else {
          await navigator.clipboard.writeText(style.fn(value));
        }
        btn.textContent='Скопировано ✓'; btn.classList.add('copied');
        setTimeout(()=>{btn.textContent='Копировать';btn.classList.remove('copied')},1200);
      } catch(e) {
        const ta=document.createElement('textarea'); ta.value=style.fn(value); document.body.appendChild(ta); ta.select(); document.execCommand('copy'); ta.remove();
